...
annotation <- read.csv("/home/dell/Documents/Arindam/ReferenceGenome/Human_Ensembl84/Annotation/gtf/GeneAnnotation.csv", header=T)
annotation.keep <- annotation[annotation$EnsemblGeneID %in% row.names(res_clean),]
res_clean <- merge(res_clean, annotation.keep[,c(1,4)], by.x=0, by.y="EnsemblGeneID")
colnames(res_clean)[1] <- "EnsemblGeneID"

upreg_deseq <- res_clean[(res_clean$log2FoldChange>1 & res_clean$padj<0.05),]
sprintf ("Number of upregulated genes (DESeq2)")
dim(upreg_deseq)
write.table(upreg_deseq, file=paste(ProjectID, "_DESeq2_upreg.tsv", sep=""), sep="\t", row.names=FALSE)

downreg_deseq <- res_clean[(res_clean$log2FoldChange<(-1) & res_clean$padj<0.05),]
sprintf ("Number of downregulated genes (DESeq2)")
dim(downreg_deseq)
write.table(downreg_deseq, file=paste(ProjectID, "_DESeq2_downreg.tsv", sep=""), sep="\t", row.names=FALSE)
