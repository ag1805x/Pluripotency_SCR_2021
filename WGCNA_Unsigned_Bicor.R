...
sft = pickSoftThreshold(datExpr, powerVector = powers, corFnc="bicor" ,verbose = 5, blockSize=20000, networkType="unsigned")
...
net <- blockwiseModules(datExpr,   maxBlockSize = 20000, power = 10, corType="bicor", networkType="unsigned", TOMType = "unsigned", minModuleSize = 30, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "NP_TOM", verbose = 3)
table(net$colors)
#sizeGrWindow(12, 9)
...
{
modGenes <- (moduleColors==module)
connectivity.modGenes <- connectivity[modGenes, ]
fileName <- paste("Connectivity-", module, ".txt", sep="");
write.table(as.data.frame(connectivity.modGenes), file = fileName, sep="\t", row.names = TRUE, col.names = NA)
}
